...
<Tabs items={["npm", "pnpm", "yarn"]} storageKey="selected-pkg-manager">
  ...
  <Tab>
  ```bash copy
  pnpm install @travelated-kuma-ui/core
  ```
  </Tab>
  {/* prettier-ignore */}
  <Tab>
  ```bash copy
  yarn add @travelated-kuma-ui/core
  ```
  </Tab>
</Tabs>

Once the `@travelated-kuma-ui/core` package is successfully installed, you can proceed to install a plugin for your specific framework (Next.js or Vite) as below.

<Tabs items={["next.js", "vite"]} storageKey="selected-framework">
  {/* prettier-ignore */}
  <Tab>
  ```bash copy
  pnpm install -D @travelated-kuma-ui/next-plugin
  ```
  </Tab>
  {/* prettier-ignore */}
  <Tab>
  ```bash copy
  pnpm install -D @travelated-kuma-ui/vite
  ```
  </Tab>
</Tabs>
...
>
  {/* prettier-ignore */}
  <Tab>
    If your project uses Storybook Webpack builder (including `@storybook/nextjs`), you can use `"@travelated-kuma-ui/webpack-plugin"`.
